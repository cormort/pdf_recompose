...
async function generatePDF() {
    ...
    try {
        ...
        try {
            ...
            // 注意：此處字型路徑改為相對路徑，您可能需要一個 'fonts' 資料夾
            // 為了簡單起見，我先用一個線上的 Noto Sans TC
            // const fontUrl = './fonts/NotoSansTC-Regular.ttf'; 
            const fontUrl = 'https://raw.githubusercontent.com/google/fonts/main/ofl/notosanstc/NotoSansTC-Regular.otf';
        ...
        let pageOffset = addToc ? 1 : 0; // 如果有目錄，內容頁碼+1
        ...
        if (addEncryptCheckbox.checked) {
            const password = prompt("請輸入 PDF 密碼（若取消則不加密）：");
            if (password && password.trim() !== "") {
                saveOptions.userPassword = password;
                saveOptions.ownerPassword = password; // 為簡單起見，使用者和擁有者密碼相同
        ...
        if (finalFileName === null) { // 使用者點選「取消」
