...
*   **🛠️ 編輯功能**：
    ...
    *   頁面旋轉 (90度/180度)。
    *   來源檔案頁面刪除。
*   **#️⃣ 頁碼格式可自訂**：支援 `{n}` 目前頁碼、`{total}` 總頁數（**含目錄頁**）、`{name}` 來源檔名、`{date}` 日期。
    位置可選六個角落（右下／正下／左下／右上／正上／左上），另有邊距與字級可調，並附四個快速套用
    （純數字、第 N 頁 / 共 M 頁、N / M、- N -）。中文頁碼走思源黑體，可正常搜尋與複製。
...
*   工作階段還原：清掉之後不會被排程中的保存寫回來、重新開啟會詢問還原、還原後檔案與編排都回來、
  選擇不還原會把工作階段刪掉。
*   頁碼格式：自訂格式每頁正確（含 `{total}` 把目錄頁算進去）、同一頁只有一個頁碼（沒有多印一份原本的純數字）、
