...
*   **🛠️ 編輯功能**：
    ...
    *   頁面旋轉 (90度/180度)。
    *   來源檔案頁面刪除。
*   **✂️ 依小節拆成多檔**：每個小節各輸出一個 PDF，打包成一個 ZIP 下載。沒有小節時整份為一群。
    檔名可用 `{n}` 編號、`{title}` 小節標題、`{total}` 總檔數、`{date}` 日期組成，標題中的檔案系統保留字元會自動替換，同名自動加序號。
    拆出的檔案會套用裁切／統一尺寸／頁面標記設定；目錄頁與書籤大綱屬於整份文件層級，不會逐檔重複產生。
*   **🖨️ 一頁多張／騎馬釘**：列印用拼版，把多頁併到一張紙上。
    *   每張頁數：2／4／6／9 up；紙張尺寸可沿用頁面或指定 A4／A5／Letter。
    *   排列方向可選「先左而右」或「先上而下」；可畫格線。
    *   **騎馬釘**：依對折順序重排頁序（頁數自動補成 4 的倍數），雙面列印後對折即為連續頁碼。
*   **📐 版面調整（裁白邊／統一尺寸）**：只改頁面框與尺寸，**不動內容**（文字、搜尋、書籤連結都保留）。
    *   **裁掉多餘白邊**：載入時逐頁掃描實際內容範圍，只留下內容加留白；掃描件與公文常見的大量白邊一鍵處理。
    *   **四邊平均內縮**：固定內縮指定點數。
    *   **統一頁面尺寸**：A4／A5／B5／Letter，等比縮放並置中，混合尺寸的來源檔也能輸出成一致版面。
    *   兩種設定可併用（先裁白邊再統一尺寸）。
*   **#️⃣ 頁面標記（頁首／頁尾／頁碼）**：三者共用同一組變數與位置邏輯，各自可獨立設定格式與位置。
    *   **分節編號**：有小節時，編號在每個小節重新起算；在目錄編輯器的行首加上 `[roman-lower]`、
        `[roman-upper]`、`[chinese]`、`[alpha-lower]`、`[alpha-upper]` 即可讓該小節改用不同方案
        （例如正文 `1,2,3`、附錄 `i,ii,iii`）。同一小節內沿用，後續頁面不必重複標記。
    *   沒有小節時維持原本行為：整份連續編號，且 `{n}` 含目錄頁位移，既有文件頁碼不會改變。
    *   變數：`{n}` 頁碼、`{total}` 總頁數（**含目錄頁**）、`{name}` 來源檔名、`{date}` 日期。
    *   頁首／頁尾留空就不印；頁碼留空會退回純數字（勾了標記通常就是要頁碼）。
    *   位置：頁首／頁尾各有三個、頁碼六個角落可選；邊距與字級三者共用。
    *   附四個頁碼快速套用（純數字、第 N 頁 / 共 M 頁、N / M、- N -）。中文走思源黑體，可正常搜尋與複製。
*   **💧 浮水印／印章**：在成品頁面疊字（例如「機密」「副本」「已核閱」），可**置中**單一或**平鋪**佈滿整頁。
    字級、透明度、旋轉角度、顏色（紅／灰／藍／黑）可調，並支援頁面範圍（`1-3,5`、`7-`、留空＝全部）。
    文字留空就不會蓋，避免產出一份被空白浮水印污染的檔案。中文走思源黑體，疊上去的字**仍可被搜尋與複製**。
...
*   工作階段還原：清掉之後不會被排程中的保存寫回來、重新開啟會詢問還原、還原後檔案與編排都回來、
  選擇不還原會把工作階段刪掉。
*   拆檔：ZIP 內檔數＝小節數、每個檔案的 **CRC 正確**、檔名套用格式（含中文）、各檔頁數符合小節範圍、
  各檔內容確實是該小節的頁面、格式改成 `{n}` 時檔名跟著變、關閉拆檔後回到單檔預覽流程。
*   拼版：2-up／4-up 的張數與格子順序正確、頁數不足時最後一格留白、輸出紙張尺寸正確、
  關閉拼版後恢復原狀；騎馬釘的頁序以**純函式單元檢查**驗證（見下）。
*   版面調整：預設維持原尺寸、「裁掉白邊」會縮到內容附近（且兩頁差異在幾 pt 內）、統一成 A4／A5 後
  尺寸真的改變、**統一尺寸後文字仍可抽取**（沒有變成圖片）、裁白邊與統一尺寸可以併用。
*   分節編號：羅馬／中文／字母／十進位的轉換以純函式單元檢查（含 `10→十`、`101→一百〇一` 這類邊界）、
  預設會隨小節重新起算、加上 `[roman-lower]` 後附錄真的變成 `i,ii`、標記能來回（重開編輯器帶得出來
  且不污染標題）、以及產出的 PDF 上確實印出 `i`／`ii`。
*   頁面標記：自訂格式每頁正確（含 `{total}` 把目錄頁算進去）、同一頁只有一個頁碼（沒有多印一份原本的純數字）、
  `{name}`／`{date}` 會代入、快速套用會改寫格式欄位、位置切換以座標驗證；
  頁首／頁尾／頁碼**同時開啟且各自不同位置**時都正確（以文字座標斷言，不是目測）。
*   浮水印：每一頁都蓋到且中文可被抽取、原本的頁面文字沒被蓋掉、平鋪模式每頁出現多個、
  頁面範圍（`2` 與 `1-2,3`）正確、文字留空時完全不蓋。
